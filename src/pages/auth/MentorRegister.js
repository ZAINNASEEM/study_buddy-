...
const MentorRegister = () => {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            // Save basic user info to 'users' collection (merge to avoid accidental overwrite)
            ...
            // Normalize expertise array and dedupe
            const expertise = formData.subjects.split(',').map(s => s.trim()).filter(Boolean);
            const uniqueExpertise = [...new Set(expertise)];

            // Save detailed mentor profile to 'mentors' collection
            ...
            setSuccess('Application submitted — we will review your profile and notify you when approved.');
            setTimeout(() => navigate('/mentor/waiting-approval'), 1000);
        } catch (err) {
            console.error("Mentor registration error:", err);
            // Provide a friendlier message for common Firebase Auth configuration issue
                ...
                <Col md={8} lg={6}>
                    <Card className="shadow-lg border-0 rounded-3">
                        <Card.Body className="p-5">
                            <h2 className="text-center fw-bold mb-2">Mentor Application</h2>
                            <p className="text-center text-muted mb-4">Share your knowledge with others</p>
                            ...
                            <Form onSubmit={handleRegister}>
                                ...
                                <Form.Group className="mb-3" controlId="subjects">
                                    <Form.Label>Area of Expertise (comma separated)</Form.Label>
                                    <Form.Control
                                        type="text"
                            ...
                            <div className="text-center mt-4">
                                <p className="text-muted">
                                    Already a mentor? <Link to="/mentor/login" className="fw-bold text-success text-decoration-none">Login Here</Link>
                                </p>
                            </div>
                        </Card.Body>
                    </Card>
